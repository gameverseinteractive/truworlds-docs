...
Some functions may return `void`, which is a special type of `nil` that indicates that a function didn't return anything. If you write a function that returns without specifying a value, it will return `void`. For example, some functions like `print()` return `void`.
[source,truscript]
----
local print_return = print("Hello, world!")
print(print_return) # prints "void"
----
